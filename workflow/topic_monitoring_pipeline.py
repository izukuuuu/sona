    ...
    def __init__(
        ...
        *,
        ...
        if db is not None:
            ...
            if use_external_db is True or (use_external_db is None and topic_store_configured()):
                self.db = get_db()
            elif resolve_topic_monitor_sqlite_path() is not None:
                self.db = TopicMonitorSqliteStore.from_env()
            else:
                self.db = _DEFAULT_MEMORY_STORE
